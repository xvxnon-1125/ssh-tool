...
本ツールは複数サーバに対する情報収集や状態確認を目的としています。

以下のような用途には適していません。

- パスワード入力や確認応答など、対話操作が必要なコマンド
- `top` や `less` など、対話型インタフェースを使用するコマンド
- `tail -f` や `journalctl -f` など、終了しないコマンド
- システム停止や再起動など、影響範囲の大きいコマンド
- 大量の出力を生成するコマンド
- 複雑な処理や条件分岐を含むスクリプト

本ツールは主に以下のような参照系（Read Only）の運用作業を想定しています。
